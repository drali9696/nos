  ...
  switch (access.dialect) {
    ...
    case 'openai':
      ...
      if (!oaiKey && oaiHost.indexOf(DEFAULT_OPENAI_HOST) !== -1)
        throw new Error('Missing Key. Add it on the UI (Models Setup) or server side (your deployment).');
    ...
    case 'openrouter':
      ...
      if (!orKey || !orHost)
        throw new Error('Missing Key or Host. Add it on the UI (Models Setup) or server side (your deployment).');
